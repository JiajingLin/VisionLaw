...
class Physics(nn.Module):
    ...
    def plasticity(self, F: torch.Tensor) -> torch.Tensor:
        """
    ...
    def elasticity(self, F_corrected: torch.Tensor) -> torch.Tensor:
        """
        Compute Kirchhoff stress tensor from corrected deformation gradient tensor.
    ...
    def forward(self, F: torch.Tensor) -> torch.Tensor:
        ...
        Returns:
            kirchhoff_stress (torch.Tensor): Kirchhoff stress tensor (B, 3, 3).
        """
        F_corrected = self.plasticity(F)
        kirchhoff_stress = self.elasticity(F_corrected)
        return kirchhoff_stress