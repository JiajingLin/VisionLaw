...
import visionlaw
from visionlaw.warp import Tape, CondTape
...
class MPMModelBuilder(ModelBuilder):
    ...
    def parse_cfg(self, cfg: visionlaw.config.physics.sim.BaseSimConfig) -> 'MPMModelBuilder':
...
class MPMInitData(object):
    ...
    @classmethod
    def get(cls, cfg) -> 'MPMInitData':
        kwargs: dict = None
        if isinstance(cfg, visionlaw.config.particles.ParticlesConfig):
            if cfg['shape']['name'] == 'pcd':
                kwargs = cls.get_pcd(
                    cfg['shape']['pcd_name'],
                    cfg['shape']['asset_root'],
                    cfg['shape']['sort'],
                    cfg['shape']['ori_bounds'],
                    cfg['shape']['sim_bounds']
                )
        else:
    ...
    def get_gaussians(
        ...
        """
        Generate a gaussian of particles with the specified center, size, resolution, and mode.
        """
        pass
    ...
    def get_cube(
        ...
        """
        Generate a cube of particles with the specified center, size, resolution, and mode.
        """
...
class MPMStateInitializer(StateInitializer):
    ...
    def finalize(self) -> tuple[StateType, list[int]]:
        """
        初始化粒子的状态，包括粒子的 位置 和 速度，并将这些状态整合到一个 MPMState 对象中
        """
        pos_groups = []
        vel_groups = []
        sections = []
