...
def get_function_from_path(path: Path | str, function_name: str) -> Any:
    path = Path(path)
    spec = importlib.util.spec_from_file_location(path.stem, path)
    module = importlib.util.module_from_spec(spec)
    spec.loader.exec_module(module)
    ...
    print(f"parameter nums: {len(sig.parameters)}")
    print(f"parameter datails:")
...
def run_exp(base_cmds: list[str], extra_args: dict[str, Any], unknown_args: list[str], env: dict[str, str]) -> str:
    ...
    if error is not None:
        ...
        match = re.search(r'RuntimeError.*', error, re.DOTALL)  
        if match:
            error = match.group(0)
            print(f'-------------------error:{error}-------------------')
        else:
...
def safe_run_exp(worker, cfg, guidance, plasticity, elasticity, gaussians, bindings, **unknown_args):
    p = mp.Process(target=worker, args=(cfg, guidance, plasticity, elasticity, gaussians, bindings), kwargs=unknown_args)
    p.start()
    p.join()
